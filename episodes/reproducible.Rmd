...
![We need to manually fix the DESCRIPTION file. Packages like {cfr} and {epiparameter} are on GitHub.](fig/add_dependencies-github.png)
...
:::hint
- If you need to install a new package and update the `{renv}` status.
- If you decide to create a new file for this, remember to add it to the `make.R` file (as in the step of the following section)
:::
...
Reproducible research is defined as a practice that wants to ensure that _anyone with access_ to data inputs and software can feasibly generate the outputs, both to check or build on them.

A key feature for this practice is the `make.R` file. With this file, and any other more sophisticated alternatives like `GNU Make` or `{targets}` we now are sure that we:
