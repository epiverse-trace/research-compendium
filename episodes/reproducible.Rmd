...
- Provides an _automated_ package management solution to restore a external project.
...
First, for __Data management__ we need to save input data as originally created and, preferably, configure it as a read-only file. 

Second, for __Project organization__, we need to store analysis and generated files in specific and isolated folders. These two practices mean to differentiate `raw-data` from `derived-data`, and differentiate `analyses` files (like `.R` scripts and `.Rmd` files) from `figures` and other `outputs`. 

<!-- add figure image of workflow -->

### Automate your analysis

The `make.R` file helps to automate the your whole analysis project. This includes a script line to automatically restore your dependencies (`renv::restore()``) and
then run all the analysis scripts in your preferred order. The `make.R` file is the only `.R` file stored in the root of the project given by the `{rcompendium}` template. The `make.R` file can also be used as the only script to run and regenerate all your project outputs.

:::callout

The `make.R` file is inspired but not equivalent to GNU `Make` file. 

[GNU `Make`](https://www.gnu.org/software/make/) files can identify out-of-date files and re-execute any downstream code that need to be updated. This is usually use for [`bash` scripts](http://book.biologistsguide2computing.com/en/stable/automation-is-your-friend.html).
...
We are going to use `{renv}` instead of `DESCRIPTION` files.

:::instructor

We opt to use `{renv}` instead of `DESCRIPTION` because the `rcompendium::add_dependencies(".")` function because it assumes that all packages to add to `DESCRIPTION` are from CRAN. If you want to add GitHub packages, [you need to manually add them](https://remotes.r-lib.org/articles/dependencies.html) in a different section called `Remotes:` and write `repository/package`. The `{renv}` package solve this automatically.

<!-- paste figure when using epiverse packages -->

However, this stills needs to be assessed with different scenarios to confirm this as the final best decision.
...
Note that this function requires one argument specification `"."`. This means that your [working directory](https://www.epirhandbook.com/en/r-basics.html?q=getwd#working-directory) must be in the root of the R project.

The output below details which packages are included in the description file
...
Second, to get an status of the project run:
...
Always follow the suggestions of the `renv::status()` output. You can also get a message from it each time you open again your project.
...
- specific version details of all the packages in the dependency tree of the project.
...
Now you completed you reproducible environment configuration. 

### The analysis workflow

The workflow will follow this three paths:
...
Second, create the analysis script to clean this raw data set, called `01-clean.R`. Save it in the `analyses/` folder. Paste these lines of code:
...
- The default folder to save R scripts will be `R/`. This is the place to write your _Modular functions_.
...
Third, create a the analysis script to create an [incidence plot](https://www.reconverse.org/incidence2/) this cleaned data set, called `02-plot.R`. Save it in the `analyses/` folder. Paste these lines of code:
...
This is the easiest step to forget!
...
_Modular functions_ must be written in the `R/` folder. To load your project functions, as written in `line 20` of the `make.R` file, run:
...
Now, Is there a easier way to navigate through the documentation for my _modular functions_? Let's wait for the next episode!
